      ...
      {p.mySkills.map((skill, idx) => {
        ...
        return (
          <div key={`mine-${skill.id}`}>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="text-[13.5px] font-bold text-brand-parchment leading-tight truncate">
                  {skill.name}
                </div>
                {skill.description && (
                  <div className="text-[11.5px] text-brand-muted leading-snug mt-0.5 truncate">
                    {skill.description}
                  </div>
                )}
              </div>
...
const PlusIntegrationsBody = (p: PlusContentProps) => (
  <div className="flex flex-col">
        ...
        {p.integrationCategories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 px-4 h-8 rounded-full text-[12.5px] font-semibold border transition-colors ${
                active
    ...
    <div className="flex flex-col gap-2 max-h-[55vh] overflow-y-auto overscroll-contain pb-3 px-0.5">
      ...
      {p.filteredIntegrations.map((it) => {
        ...
        return (
          <motion.button
            ...
            disabled={isLoading}
            className="w-full flex items-center gap-3.5 px-3 py-3 rounded-[18px] bg-foreground/[0.05] border border-foreground/12 transition-colors active:bg-foreground/[0.08] text-left"
          >
            <div className="w-9 h-9 flex items-center justify-center overflow-hidden shrink-0 rounded-lg bg-foreground/10 border border-foreground/15">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11.5px] text-brand-muted font-medium truncate leading-tight mt-0.5">
                {it.description}
              </div>
            </div>
            {connected ? (
              ...
              <span className="shrink-0 inline-flex items-center justify-center h-7 px-3 rounded-full bg-foreground/10 text-foreground border border-foreground/20 text-[11.5px] font-semibold">
                Connect
              </span>
            )}
          </motion.button>
...
const PlusTools = (p: PlusContentProps) => {
  ...
  return (
    <motion.div key={isSkills ? "skills" : "integrations"} {...fadeProps(12)} className="flex min-h-0 flex-col">
      <div className="flex items-center gap-2 px-1 pt-1 pb-2">
        <motion.button
          whileTap={{ scale: 0.92 }}
          onClick={() => p.setPlusView("main")}
