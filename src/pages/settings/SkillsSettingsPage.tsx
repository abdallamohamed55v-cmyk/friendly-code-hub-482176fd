...
export default function SkillsSettingsPage() {
  ...
  return (
    <SubShell
      title="Skills"
      subtitle="Design experts Megsy can switch into inside chat. Enable, edit, or import your own."
      action={
        <button
          onClick={() => navigate("/settings/skills/new")}
          className="hidden sm:inline-flex items-center gap-1.5 h-9 px-3.5 rounded-full bg-foreground text-background text-[12.5px] font-medium hover:opacity-90 transition"
        >
    ...
    >
      ...
      <SubSection
        title="Active roster"
        description="Enabled skills are available to Megsy inside every chat."
      >
        <SubStatStrip
          items={[
            { label: "Enabled", value: String(enabledCount) },
            { label: "Total", value: String(totalCount) },
            { label: "Library", value: String(librarySkills.length) },
          ]}
        />
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          >
            <Plus className="w-3.5 h-3.5" strokeWidth={2.2} /> Create skill
          </button>
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={importing}
            className="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-full border border-border/70 bg-card/40 text-[12.5px] font-medium text-foreground hover:bg-foreground/[0.05] disabled:opacity-50 transition"
          >
      ...
      <SubSection
        title={tab === "mine" ? "Your skills" : "Library"}
        description="Switch between skills you've made and curated starters from the Megsy library."
      >
        <div className="mb-3 inline-flex items-center gap-1 p-1 rounded-xl bg-foreground/[0.04] border border-border/60">
          {[
            ...
            return (
              <button
                ...
                onClick={() => setTab(t.id)}
                className={cn(
                  "flex items-center gap-2 px-3.5 h-8 text-[12.5px] font-medium rounded-lg transition-all",
                  active
              ...
              >
                ...
                <span
                  className={cn(
                    "min-w-[18px] h-[16px] inline-flex items-center justify-center text-[10px] font-semibold px-1.5 rounded-full",
                    active
        ...
        {tab === "mine" ? (
          loading ? (
            <SubCard className="flex items-center justify-center py-14">
              <Loader2 className="w-5 h-5 animate-spin text-muted-foreground" />
            </SubCard>
          ) : mySkills.length === 0 ? (
            <SubCard className="text-center py-14">
              <p className="text-[14.5px] font-medium text-foreground">No skills yet</p>
              ...
              <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-full border border-border/70 text-[12.5px] font-medium text-foreground hover:bg-foreground/[0.05] transition"
                >
                  Browse library
                </button>
              </div>
          ...
          <SubCard className="text-center py-14">
            <p className="text-[14.5px] font-medium text-foreground">The library is empty</p>
                ...
                return (
                  <div key={s.id} className="flex items-start gap-3 px-4 py-3.5">
                    ...
                    <button
                      ...
                      className={cn(
                        "shrink-0 inline-flex items-center gap-1 h-8 px-3 rounded-full text-[12px] font-medium transition",
                        exists
      ...
      <SubSection
        title="Inspiration"
        description="Not sure where to start? Try one of these prompts."
      >
        <div className="flex flex-wrap gap-2">
          {SUGGESTIONS.slice(0, 6).map((s) => (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 h-8 rounded-full border border-border/70 bg-card/40 text-[12.5px] text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
            >
...
function SkillRow({
  ...
  return (
    <div
      className={cn(
        "group flex items-start gap-3 px-4 py-3.5 transition-colors hover:bg-foreground/[0.03]",
        enabled ? "" : "opacity-50",
      )}
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          />
          <p className="text-[14px] font-medium text-foreground truncate">{skill.name}</p>
        </div>
        {skill.description && (
          <p className="text-[12.5px] text-muted-foreground line-clamp-2 mt-0.5 leading-snug">
            {skill.description}
        ...
        {skill.triggers && skill.triggers.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {skill.triggers.slice(0, 4).map((t, i) => (
              <span
                key={i}
                className="inline-flex items-center h-5 px-1.5 rounded bg-foreground/[0.06] text-[10.5px] font-medium text-muted-foreground"
              >
                /{t}
              </span>
            ))}
            {skill.triggers.length > 4 && (
              <span className="inline-flex items-center h-5 px-1.5 rounded bg-foreground/[0.06] text-[10.5px] font-medium text-muted-foreground">
                +{skill.triggers.length - 4}
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-1">
        <button
          onClick={onEdit}
          className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-foreground/[0.06] opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all"
          aria-label="Edit"
        >
        ...
        <button
          onClick={onDelete}
          className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all"
          aria-label="Delete"
        >
