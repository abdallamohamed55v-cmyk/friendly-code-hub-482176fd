    ...
    <CardShell tone="emerald" label={tt.question_choose}>
      ...
      <div className="flex flex-col gap-2">
        {(card.options || []).map((opt: string, i: number) => {
          ...
          let cls =
            "border-border/60 bg-card hover:border-emerald-300/60 hover:bg-emerald-500/[0.04] text-foreground/80";
          let chipCls =
            "bg-muted/60 text-muted-foreground group-hover:bg-emerald-500/10 group-hover:text-emerald-600 dark:group-hover:text-emerald-300";
          if (isAnswerRevealed) {
            if (isCorrectOpt) {
              cls =
                "border-emerald-400/70 bg-emerald-500/[0.08] text-emerald-700 dark:text-emerald-200";
          ...
          return (
            ...
            >
              <span
                className={`w-6 h-6 rounded-lg flex items-center justify-center text-[11px] font-bold shrink-0 transition-colors ${chipCls}`}
              >
                {letter}
              </span>
      ...
      {picked !== null && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
          <div
            className={`text-[11px] font-medium ${isRight ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}
          >
    ...
    <CardShell tone="emerald" label={tt.select_all_correct}>
      ...
      {!submitted ? (
        ...
        <div className="space-y-2">
          <div
            className={`text-xs ${allRight ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}
          >
            {allRight ? tt.all_correct : tt.not_all_correct}
          </div>
          {card.explain && (
            <div className="text-xs text-muted-foreground border-r-2 border-emerald-400/40 pr-3">
              {card.explain}
            </div>
          )}
          <TeacherNoteInput locale={loc} onSend={(t) => onAnswer?.(t)} />
  ...
  return (
    <CardShell tone="emerald" label={tt.true_or_false}>
      ...
      {picked !== null && (
        <div className="space-y-2">
          <div
            className={`text-xs ${isRight ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}
          >
    ...
    <CardShell tone="emerald" label={tt.fill_blank}>
      ...
      {sent && (
        <div className="space-y-1.5">
          ...
          >
            {isRight
              ? tt.correct_with_answer(String(card.answer))
              : tt.correct_answer_is(String(card.answer))}
          </div>
      ...
      <div className="space-y-2">
        {pairs.map((p: any, i: number) => (
          <div key={i} className="flex items-center gap-2">
            <span className="flex-1 px-3 py-2 rounded-lg bg-background/60 border border-border/40 text-sm">
              {p.a}
            </span>
...
const MermaidCard = ({ card }: BaseProps) => {
  ...
  return (
    <CardShell tone="violet" label={tt.visual_explanation}>
      ...
      <div
        ref={ref}
        className="overflow-x-auto rounded-lg bg-background/40 p-2 [&_svg]:max-w-full [&_svg]:h-auto"
      />
      {err && (
    ...
    <CardShell tone="blue" label={tt.learning_map}>
      ...
      <div className="space-y-2">
        {stages.map((s: any, i: number) => (
          <div
            key={i}
            className="rounded-xl border border-border/40 bg-background/60 p-3 space-y-1.5"
          >
            <div className="flex items-center gap-2">
            ...
            {s.resources && s.resources.length > 0 && (
              <div className="flex flex-wrap gap-1 pr-8">
                {s.resources.map((r: string, k: number) => (
                  <span
                    key={k}
                    className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-700 dark:text-blue-300"
                  >
                    {r}
                  </span>
                ))}
            ...
            {s.project && (
              <div className="pr-8 text-[11px] text-emerald-700 dark:text-emerald-300">
                Project: {s.project}
              </div>
            )}
            <button
        ...
        <div className="space-y-1.5">
          {questions.map((q: any, i: number) => (
            <div key={i} className="flex items-start gap-2 text-xs">
              <span
                className={`text-[11px] font-semibold shrink-0 mt-0.5 ${answers[i] === q.correct ? "text-emerald-600 dark:text-emerald-300" : "text-rose-600 dark:text-rose-300"}`}
              >
  ...
  return (
    <CardShell tone="rose" label={tt.question_n_of_m(idx + 1, questions.length)}>
      <div className="flex items-center justify-between text-xs">
        ...
        <span
          className={`font-mono font-semibold ${remaining < 60 ? "text-rose-700 dark:text-rose-300" : "text-foreground"}`}
        >
          {min}:{sec}
        </span>
      </div>
      ...
      <div className="flex gap-2">
        {idx > 0 && (
          <button
            ...
            className="flex-1 py-1.5 rounded-lg bg-background/60 border border-border/40 text-xs"
          >
            {tt.previous}
          </button>
        )}
