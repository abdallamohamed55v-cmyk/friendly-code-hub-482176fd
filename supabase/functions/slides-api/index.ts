  ...
  try {
    switch (action) {
      ...
      case "images_pdf": {
        ...
        // Allowlist trusted PDF sources only (prevents SSRF to internal / metadata endpoints).
        ...
        try { parsedTarget = new URL(target); } catch { return err("Invalid `url`"); }
        if (parsedTarget.protocol !== "https:") return err("Invalid `url`: https required");
        if (!ALLOWED_PDF_HOSTS.has(parsedTarget.hostname)) {
          return err("Invalid `url`: host not allowed", 403);
        }
